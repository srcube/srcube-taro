...
function ComponentLinks(props: ComponentLinksProps) {
  const { npm, taro, source } = props

  return (
    <div className="not-prose flex flex-wrap gap-3 mt-6">
      {npm && (
        <ButtonLink
          href={`https://www.npmjs.com/package/@srcube-taro/${npm}`}
      ...
      {taro && (
        <ButtonLink
          href={`https://taro-docs.jd.com/docs/components/${taro}`}
      ...
      {source && (
        <ButtonLink
          href={`https://github.com/srcube/srcube-taro/tree/main/packages/ui/${source}`}

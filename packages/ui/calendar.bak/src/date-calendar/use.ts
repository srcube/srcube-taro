...
export function useDateCalendar(props: UseDateCalendarProps) {
  ...
  const context = useMemo<CalendarContextType<CalendarState>>(() => ({
    ...
    targetMonthIndex,
    setTargetMonthIndex,
  }), [state, slots, styles, locale, weekdayStyle, firstDayOfWeek, size, isRange, isYMPickerExpanded, setIsYMPickerExpanded, months, monthIndexMap, activeMonthIndex, setActiveMonthIndex, targetMonthIndex, setTargetMonthIndex])
